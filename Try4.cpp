 ...
 * 		variable, guarded by a mutex, from local count variables in each thread while using
 *		padding.
...
const int CACHE_LINE = 64; // Found in /sys/devices/system/cpu/cpu0/cache/index0/coherency_line_size

struct padded_int
{
	int value;
	char padding[CACHE_LINE - THREADS]; 
} private_count[THREADS];
...
void create_array(int * myArray)
{
	...
	std::uniform_int_distribution<> distr(generatorMin, generatorMax);
